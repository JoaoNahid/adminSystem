        ...
        <div className="menuPaginasEdicao">
            {
                paginasEditaveis.map((pagina, index) => {
                    return (
                        <div key={index} id={'paginaMenu' + index} value={index} onClick={(index) => setMenuLateral(document.getElementById('paginaMenu' + index).value)} className="paginaEdicao" name="SideMenuHome">{pagina.pagina}</div>
                    )
                })
            }
        </div>
